...
class Validate {
    ...
    public static function isCityComma($city) {
        return preg_match('/^[0-9]+([,][0-9]+)*$/', $city);
    }
...
//return ctype_digit($time) && $time <= 2147483647;
    ...
    public static function isUnsignedInt($value) {
        return (preg_match('#^[0-9]+$#', (string) $value) && $value < 4294967296 && $value >= 1);
    }
    ...
    public static function isOverMaxLen($str='', $len=0, $lan='B'){
        if($lan=='S'){
    ...
    public static function isLowMinLen($str='', $len=0, $lan='B'){
        if($lan=='S'){
            $strlen=mb_strlen($str);
        }else{
            $strlen=strlen($str);
