...
<div class="search-nav">
	<form class="form-inline" name="searchform" action="<?php echo Url::toRoute(['/sys/menu/index']); ?>" method="post">
		...
		<div class="form-group input-group-sm">
			...
			<button class="btn btn-default" type="button" onclick="Dialog('','添加菜单','<?php echo Url::toRoute('/sys/menu/add'); ?>', 620, 300)"><i class="glyphicon glyphicon-plus-sign"></i> 添加菜单</button>
		</div>
	</form>
</div>
...
<script type="text/javascript">
	function createsonmenu(id) {
		Dialog('','添加子菜单','<?php echo Url::toRoute(['/sys/menu/addchildren'])?>?id='+id, 620, 330);
	}
	//编辑菜单信息
	function edit(id, username) {
		Dialog('','修改菜单-'+username+'信息','<?php echo Url::toRoute(['/sys/menu/edit'])?>?id='+id, 620, 300);
	}
</script>
