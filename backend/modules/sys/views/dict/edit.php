...
<form action="<?php echo Url::to(['/sys/dict/edit','id'=>$info->id]) ?>" method="post" id="myform">
    <table width="100%" class="table_form contentWrap">
        <tr>
            <th width="100">编码：</th>
            <td><input type="text" value="<?=$info->code?>" name="Dict[code]" id="code" class="input-text"></td>
        </tr>      
        <tr>
            <th>标签：</th>
            <td><input type="text" value="<?=$info->label?>" name="Dict[label]" id="label" class="input-text"></td>
        </tr>
        <tr>
            <th>字典组：</th>
            <td>
                <select id="dict_group_id" name="Dict[dict_group_id]" class="select-option">
                    ...
                    <?php foreach (DictGroup::find()->select(['id', 'name'])->all() as $list): ?>
                    <option <?php if($info->dict_group_id == $list->id):?>selected=""<?php endif;?> value="<?= $list->id; ?>"><?= $list->name; ?></option>
                    <?php endforeach; ?>
                </select>
            </td>
        </tr>
